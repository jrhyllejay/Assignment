...
def searchRecord():
    ...
    if search == 'S':
        ...
        print(
            f'{i} \t | {students[i].getType()} \t | {students[i].getName()} \t | {students[i].getID()} \t | {students[i].getYrCourseSec()} \t | {students[i].getAverage()}')

    elif search == 'T':
        i = int(input('Enter index: '))
        print(
            f'{i} \t | {teachers[i].getType()} \t | {teachers[i].getName()} \t | {teachers[i].getID()} \t | {teachers[i].getDepPos()} \t | {teachers[i].getSub()}')
...
def displayRecords():
    ...
    if display == 'S':
        ...
        for s in students:
            print(
                f'{i} \t | {s.getType()} \t | {s.getName()} \t | {s.getID()} \t | {s.getYrCourseSec()} \t | {s.getAverage()}')
            i += 1
        print('----------------------------------------------------------------------------------')
